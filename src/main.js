...
async function dispatch(event) {
  switch (event.type) {
        ...
        prediction.querySelectorAll('button').forEach(button => {
          button.onclick = async () => {
            ...
            prediction.querySelectorAll('button').forEach(item => {
              item.disabled = true;
              const notChosen = item.dataset.choice !== choice;
              item.classList.toggle('choice-not-selected', notChosen);
            });
            resolve();
          };
        });
    ...
    case 'hideChoices':
      ...
      prediction.querySelectorAll('button').forEach(item => { item.disabled = false; item.hidden = false; item.classList.remove('choice-not-selected'); });
      break;
